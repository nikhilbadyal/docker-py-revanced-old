...
class Downloader:
    ...
    def apkmirror(cls, version: str, music: bool) -> None:
        ...
        resp = session.get(
            'https://www.apkmirror.com' + parser.css_first('a.accent_bg').attributes[
                'href'])
        parser = LexborHTMLParser(resp.text)

        href = parser.css_first(
            'p.notes:nth-child(3) > span:nth-child(1) > a:nth-child(1)').attributes[
            'href']
...
def main():
    ...
    def get_patches():
        ...
        for i, v in enumerate(app_patches):
            arg_parser.include(
                v['name']) if i in selected_patches else arg_parser.exclude(v['name'])
