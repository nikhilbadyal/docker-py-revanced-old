...
class Downloader:
    ...
    def apkmirror(cls, version: str, music: bool) -> None:
        print("Trying to download youtube apk from apkmirror")
        app = "youtube-music" if music else "youtube"
        version = "-".join(
            v.zfill(2 if i else 0) for i, v in enumerate(version.split("."))
        )
        print(f"Version for youtube to download  from apkmirror is {version}")
...
class Patches:
    ...
    def get(self, music: bool) -> Tuple[List[Dict[str, str]], str]:
        print("Getting patches for youtube")
...
class ArgParser:
    ...
    def run(cls, output: str = "revanced.apk") -> None:
        print("Sending request to revanced cli for building youtube revanced")
        args = [
            "-jar",
            "cli.jar",
            "-a",
